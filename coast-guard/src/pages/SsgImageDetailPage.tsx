...
import { useSsgImageInspect } from '../api/hooks';
import Breadcrumb from '../components/Breadcrumb';
import Section from '../components/Section';
import KeyValue from '../components/KeyValue';

/**
 * SSG flavor of {@link ImageDetailPage}: same UI shell
 * (Breadcrumb / Overview / Config / Environment / Labels / Used
 * by Services / Layer History) but reads from
 * `GET /api/v1/ssg/images/inspect` so it shows the inner Docker
 * daemon's view of an image inside the per-project SSG outer DinD.
 */
...
/**
 * Best-effort: extract the inner-compose service name from a
 * container name like `cg-ssg-postgres-1` → `postgres`. The SSG
 * outer DinD names its inner containers `<project>-ssg-<svc>-N`.
 */
...
export default function SsgImageDetailPage() {
    ...
    const { data, isLoading, error } = useSsgImageInspect(project, imageId);
    ...
    const config =
        inspect != null
            ? (safeGet(inspect, 'Config') as Record<string, unknown> | undefined)
            : undefined;
        ...
        <div className="page-shell">
            <Breadcrumb
                items={[
                    ...
                    { label: t('tab.images'), to: `/project/${project}/ssg/local/images` },
                    { label: truncateId(imageId) },
                ]}
            />

            {isLoading && <p className="text-sm text-subtle-ui py-8">{t('images.loading')}</p>}
            ...
            {inspect != null && (
                <>
                    ...
                    <Section title={t('images.overview')}>
                        <KeyValue
                            label={t('images.architecture')}
                            value={asString(safeGet(inspect, 'Architecture'))}
                        />
                        ...
                        <KeyValue
                            label={t('images.size')}
                            value={formatBytes(
                                typeof inspect['Size'] === 'number' ? inspect['Size'] : 0,
                            )}
                        />
                        <KeyValue
                            label={t('images.dockerVersion')}
                            value={asString(safeGet(inspect, 'DockerVersion'))}
                        />
                        <KeyValue
                            label={t('images.author')}
                            value={asString(safeGet(inspect, 'Author'))}
                        />
                    </Section>
                                ...
                                {history.map((layer, i) => (
                                    ...
                                    >
                                        <div className="flex items-center gap-2 mb-0.5">
                                            <span className="font-mono text-subtle-ui">
                                                #{history.length - i}
                                            </span>
                                            {layer.size > 0 && (
