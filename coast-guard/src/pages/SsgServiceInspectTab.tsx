...
import { useSsgServiceInspect } from '../api/hooks';
...
/**
 * SSG flavor of {@link ServiceInspectTab}: identical render logic
 * (State / Config / Network / Mounts / Environment / Labels) but
 * reads from `GET /api/v1/ssg/services/inspect`. Image links jump
 * to `/project/<p>/ssg/local/images/<id>` so the user stays
 * inside the SSG nav when drilling in.
 */
...
export default function SsgServiceInspectTab({ project, service }: Props) {
    const { t } = useTranslation();
    const navigate = useNavigate();
    const { data, isLoading, error } = useSsgServiceInspect(project, service);
    ...
    const networkSettings = safeGet(inspect, 'NetworkSettings') as
        ...
        <div className="flex flex-col gap-0">
            {state != null && (
                <Section title={t('inspect.state')}>
                    ...
                    <KeyValue
                        label={t('inspect.finishedAt')}
                        value={asString(state['FinishedAt'])}
                    />
                </Section>
            )}
                ...
                <Section title={t('inspect.config')}>
                    {asString(safeGet(config, 'Image')).length > 0 && (
                        <div className="flex gap-3 py-1.5 border-b border-[var(--border)] last:border-0">
                            <span className="text-xs text-subtle-ui w-40 shrink-0 font-medium">
                                {t('inspect.image')}
                            </span>
                            <span
                                ...
                                onClick={() =>
                                    navigate(
                                        `/project/${project}/ssg/local/images/${encodeURIComponent(asString(safeGet(config, 'Image')))}`,
                                    )
                                }
                            >
                    ...
                    <KeyValue
                        label={t('inspect.cmd')}
                        value={asStringArray(safeGet(config, 'Cmd')).join(' ')}
                    />
                    <KeyValue
                        label={t('inspect.entrypoint')}
                        value={asStringArray(safeGet(config, 'Entrypoint')).join(' ')}
                    />
                    <KeyValue
                        label={t('inspect.workingDir')}
                        value={asString(safeGet(config, 'WorkingDir'))}
                    />
                    <KeyValue
                        label={t('inspect.exposedPorts')}
            ...
            {networkSettings != null && (
                <Section title={t('inspect.network')}>
                    <KeyValue
                        label={t('inspect.networkMode')}
                        value={asString(safeGet(hostConfig, 'NetworkMode'))}
                    />
                    <KeyValue
                        label={t('inspect.ipAddress')}
                        value={asString(safeGet(networkSettings, 'IPAddress'))}
                    />
                    <KeyValue
                        label={t('inspect.gateway')}
                        value={asString(safeGet(networkSettings, 'Gateway'))}
                    />
                    <KeyValue
                        label={t('inspect.macAddress')}
                        value={asString(safeGet(networkSettings, 'MacAddress'))}
                    />
                    {(() => {
                        const networks = safeGet(networkSettings, 'Networks') as
                            | Record<string, unknown>
                            | undefined;
                        ...
                        return Object.entries(networks).map(([netName, netVal]) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <KeyValue
                                        label={t('inspect.ipAddress')}
                                        value={asString(safeGet(net, 'IPAddress'))}
                                    />
                                    <KeyValue
                                        label={t('inspect.gateway')}
                                        value={asString(safeGet(net, 'Gateway'))}
                                    />
                                    <KeyValue
                                        label={t('inspect.macAddress')}
                                        value={asString(safeGet(net, 'MacAddress'))}
                                    />
                                    <KeyValue
                                        label={t('inspect.networkId')}
                                        value={asString(safeGet(net, 'NetworkID'))}
                                    />
                                </div>
                            );
                        });
                        ...
                        <table className="w-full text-xs">
                            <thead>
                                <tr className="border-b border-[var(--border)]">
                                    <th className="text-left py-1.5 pr-3 text-subtle-ui font-semibold uppercase tracking-wider">
                                        {t('inspect.mountType')}
                                    </th>
                                    <th className="text-left py-1.5 pr-3 text-subtle-ui font-semibold uppercase tracking-wider">
                                        {t('inspect.mountSource')}
                                    </th>
                                    <th className="text-left py-1.5 pr-3 text-subtle-ui font-semibold uppercase tracking-wider">
                                        {t('inspect.mountDestination')}
                                    </th>
                                    <th className="text-left py-1.5 pr-3 text-subtle-ui font-semibold uppercase tracking-wider">
                                        {t('inspect.mountMode')}
                                    </th>
                            ...
                            <tbody>
                                {mounts.map((m, i) => (
                                    ...
                                    >
                                        ...
                                        <td className="py-1.5 pr-3 font-mono text-subtle-ui break-all">
                                            {m.Source}
                                        </td>
                                        <td className="py-1.5 pr-3 font-mono text-main">
                                            {m.Destination}
                                        </td>
                                        <td className="py-1.5 pr-3 font-mono text-subtle-ui">
                                            {m.Mode || '—'}
                                        </td>
                                        <td className="py-1.5 font-mono text-main">
                                            {m.RW ? t('inspect.yes') : t('inspect.no')}
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
                ...
                <Section title={t('inspect.labels')}>
                    <div className="max-h-60 overflow-auto">
                        <table className="w-full text-xs">
                            ...
                            <tbody>
                                {Object.entries(asRecord(safeGet(config, 'Labels'))).map(
                                    ([key, val]) => (
                                        <tr
                                            key={key}
                                            className="border-b border-[var(--border)] last:border-0"
