...
Doctor は状態データベースと Docker をスキャンし、不整合を検出します: コンテナが欠落している孤立したインスタンスレコード、状態レコードのない宙ぶらりんのコンテナ、実際には停止しているのに実行中とマークされた共有サービス。見つかったものは自動的に修復します。
...
[Shared Service Group](../shared_service_groups/README.md) の Postgres（または MySQL / MariaDB / MongoDB）が起動時に「data directory has wrong ownership」エラーで終了する場合、ホストの bind-mount ディレクトリが誤った UID/GID で作成されています。次を実行してください:

```bash
coast ssg doctor
```

これは `(service, host-bind-mount)` の組ごとに 1 件の結果を報告し、不一致を修正するための正確な `chown` コマンドを含めます。Doctor は読み取り専用であり、ディレクトリ自体を変更することは決してありません。完全な仕様については [SSG Volumes](../shared_service_groups/VOLUMES.md#coast-ssg-doctor) を参照してください。
