...
## 関連項目: Shared Service Groups

インライン共有サービスは 1 つのプロジェクト内では問題なくスケールしますが、プロジェクトをまたぐと衝突します。たとえば、どちらも `postgres:5432` を宣言する 2 つの Coastfile は、同じホスト上で同時には実行できません。[Shared Service Groups](../shared_service_groups/README.md) は、このパターンを複数プロジェクトにまたがって一般化するもので、`[shared_services.<name>] from_group = true` を通じて参照するすべてのプロジェクト向けに、Postgres、Redis、MongoDB などをホストする単一の SSG DinD を実行します。

インライン共有サービスから SSG へ移行すべきタイミング:

- 同じインフラサービスを必要とする複数のプロジェクトを実行していて、それらでデータを共有したい。
- 各プロジェクトが同じ標準ポートを宣言しているため、プロジェクト間でホストポート競合が発生している。
- プロジェクトをまたいで、インフライメージ、ボリューム、ライフサイクルを一元管理したい。

移行はサービスごとのオプトインです。既存のインライン `[shared_services.*]` ブロックは、そのまま変更なしで引き続き動作します。
