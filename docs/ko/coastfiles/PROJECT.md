...
Coastfile의 모든 문자열 값은 `${VAR}` 및 `${VAR:-default}` 구문을 사용한 [환경 변수 보간](VARIABLES.md)을 지원합니다.

## `[coast]`

### `name` (필수)

프로젝트의 고유 식별자입니다. 컨테이너 이름, 볼륨 이름, 상태 추적, CLI 출력에 사용됩니다. Coastfile 없이 빌드할 때는 `--name` CLI 플래그로도 지정할 수 있습니다.
...
Docker Compose 파일의 경로입니다. 상대 경로는 프로젝트 루트(Coastfile이 들어 있는 디렉터리, 또는 `root`가 설정된 경우 그 디렉터리)를 기준으로 해석됩니다. `--compose` CLI 플래그로도 지정할 수 있습니다.
