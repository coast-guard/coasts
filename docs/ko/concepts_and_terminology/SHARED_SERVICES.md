...
인라인 공유 서비스는 한 프로젝트 내에서는 잘 확장되지만 프로젝트 간에는 충돌합니다. 둘 다 `postgres:5432`를 선언하는 두 Coastfile은 같은 호스트에서 동시에 실행될 수 없습니다. [공유 서비스 그룹](../shared_service_groups/README.md)은 `[shared_services.<name>] from_group = true`를 통해 이를 참조하는 모든 프로젝트를 위해 Postgres, Redis, MongoDB 등을 호스팅하는 단일 SSG DinD를 실행함으로써 이 패턴을 프로젝트 전반으로 일반화합니다.

인라인 공유 서비스에서 SSG로 마이그레이션해야 하는 경우:

- 동일한 인프라 서비스를 필요로 하는 프로젝트를 둘 이상 실행하며, 이들이 데이터를 공유하길 원하는 경우.
- 각 프로젝트가 동일한 표준 포트를 선언해서 프로젝트 간 호스트 포트 충돌이 발생하는 경우.
- 프로젝트 전반의 인프라 이미지, 볼륨, 라이프사이클을 관리할 단일한 장소를 원하는 경우.

마이그레이션은 서비스별로 선택적으로 적용할 수 있습니다. 기존 인라인 `[shared_services.*]` 블록은 변경 없이 계속 동작합니다.
