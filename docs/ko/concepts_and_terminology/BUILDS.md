...
CLI 플래그로 구성을 직접 전달하면 Coastfile 없이도 프로젝트를 빌드할 수 있습니다:

```bash
coast build --name my-project --compose ./docker-compose.yml
```

Coastfile이 없을 때 필요한 플래그:
- `--name <NAME>` -- 프로젝트 이름
- `--compose <PATH>` -- docker-compose 파일 경로

일반적인 설정을 위한 추가 플래그:
- `--port NAME=PORT` -- 포트 매핑(반복 가능)
- `--runtime <dind|sysbox|podman>` -- 컨테이너 런타임
- `--no-autostart` -- compose 서비스 자동 시작 비활성화
- `--primary-port <NAME>` -- 기본 포트 서비스 이름

복잡한 구성(secrets, volumes, shared services)의 경우, 인라인 TOML과 함께 `--config`를 사용하세요:
...
디스크에 Coastfile이 있으면 CLI 플래그가 그 값을 덮어씁니다. Coastfile은 기본 구성을 제공하고, 플래그가 우선합니다:

```bash
coast build --name custom-name --port api=9090
```

이 명령은 기존 Coastfile을 읽지만 `coast.name`을 `custom-name`으로 바꾸고 `api` 포트를 추가(또는 덮어쓰기)합니다.
...
기본적으로 `coast build`는 Coastfile의 부모 디렉터리에서 프로젝트를 등록합니다. `--working-dir` 플래그는 이를 덮어써서, 빌드의 등록된 프로젝트 루트를 Coastfile 위치와 분리합니다:

```bash
coast --working-dir /home/user/my-project build -f /ci/configs/Coastfile
```

이 명령은 `/ci/configs/Coastfile`에 있는 Coastfile을 사용해 빌드하지만, 프로젝트 루트는 `/home/user/my-project`로 등록합니다. manifest에 저장된 `project_root`는 `coast lookup`이 인스턴스를 어디에서 매칭할지 결정하므로, `/home/user/my-project`에서 `coast lookup`을 실행하면 이 빌드의 인스턴스를 찾게 됩니다.

`--working-dir`는 상대 경로와 절대 경로를 모두 허용합니다. 상대 경로는 현재 디렉터리를 기준으로 해석됩니다.

이 기능은 CI 파이프라인, 모노레포 설정, 또는 Coastfile이 프로젝트 소스와 다른 디렉터리에 있는 모든 시나리오에 유용합니다.
