...
内联共享服务在单个项目内扩展良好，但会在跨项目时发生冲突:两个都声明 `postgres:5432` 的 Coastfile 无法同时在同一台主机上运行。[Shared Service Groups](../shared_service_groups/README.md) 将这一模式泛化到跨项目场景:运行一个单例 SSG DinD，为每个通过 `[shared_services.<name>] from_group = true` 引用它的项目托管 Postgres、Redis、MongoDB 等服务。

何时从内联共享服务迁移到 SSG:

- 你运行多个需要相同基础设施服务的项目，并希望它们共享数据。
- 你遇到了项目之间的主机端口冲突，因为它们都声明了相同的规范端口。
- 你希望有一个统一的位置来管理跨项目的基础设施镜像、卷和生命周期。

迁移可以按服务选择性进行。现有的内联 `[shared_services.*]` 块将继续保持不变地工作。
