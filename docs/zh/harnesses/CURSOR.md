...
checkout 中工作，它的 Parallel Agents 功能也可以在 `~/.cursor/worktrees/<project-name>/` 下创建 git
worktree。
...
如果你使用 Parallel Agents，请将 `~/.cursor/worktrees/<project-name>` 添加到
`worktree_dir`:
...
- **Bind mount** — 对于 Parallel Agents，Coasts 会将
  `~/.cursor/worktrees/<project-name>` 挂载到容器中的
